...
export default (staff, ingredients, additionalIngredients, order, dish, updateOrder, updateCurrentOrder)=>{

    const newIngredients=ingredients.map((item)=>item._id);
    const newAdditionalIngredients=additionalIngredients.map((item)=>item._id);
    ...
    const {_id, table, orderItems, newOrderItems, orderPrice, created_at, onKitchen, completed} = order;
